...
export const getEmailConfigStatus = () => {
  ...
  return {
    ...
    senderName: process.env.GMAIL_SENDER_NAME || 'BookMe',
    mode: 'smtp',
...
const sendWithGmail = async ({ to, subject, text, htmlContent, senderName, replyTo }) => {
  const status = getEmailConfigStatus();
  if (!status.configured) {
    throw new Error(`Gmail email is not configured. Missing: ${status.missing.join(', ')}`);
  }

  const transporter = getTransporter();
  const fromName = senderName || process.env.GMAIL_SENDER_NAME || 'BookMe';
  ...
  if (normalizedReplyTo) {
    mailOptions.replyTo = `"${normalizedReplyTo.name}" <${normalizedReplyTo.email}>`;
  }
...
export const sendTransactionalEmail = sendWithGmail;
